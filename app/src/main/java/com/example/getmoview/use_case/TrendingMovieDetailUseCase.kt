...
class TrendingMovieDetailUseCase @Inject constructor(
    private val repository: MovieRepository
) {
    operator fun invoke(movieId:Int): Flow<Resources<TrendingMoviesEntity>> = flow {
        try {
            emit(Resources.IsLoading())
            val movie = repository.getTrendingMovieById(movieId)
            Log.d("---------->", "invoke: $movie")
